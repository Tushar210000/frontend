...
export default function Janarogycard() {
    ...
    async function checkExists() {
      try {
        ...
        const res = await fetch(
          "https://ruwa-back-2.onrender.com/api/services/janarogya/check",
          {
            method: "GET",
            headers: {
 ...
 useEffect(() => {
  ...
  fetch("https://ruwa-back-2.onrender.com/api/auth/profile", {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(
          "https://ruwa-back-2.onrender.com/api/services/janarogya/user/apply",
          {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` },
            body: form,
