...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await fetch("https://backendnow-pem2.onrender.com/api/auth/profile", {
          headers: {
      ...
      <div className="card patient-card p-4 shadow-sm mt-3">
        ...
        <div className="row profile-details">
          <div className="col-md-6">
            <h6>Personal Information</h6>
            <ul>
              <li><strong>First Name:</strong> {editable ? <input name="firstName" value={formData.name} onChange={handleChange} /> : formData.firstName}</li>
